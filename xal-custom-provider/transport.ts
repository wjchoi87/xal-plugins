...
import { resolveProfile, type ProfileSettings } from "./profile";
import {
  asNumber,
  asString,
  isJsonObject,
...
function streamError(error: unknown, signal?: AbortSignal): never {
  if (
    signal?.aborted ||
  ...
  throw new ProviderError(`${PROVIDER_NAME} stream failed: ${String(error)}`, {
    retryable: true,
  });
}
...
function reasoningItem(model: string, reasoning: string): ReasoningItem {
  return {
    type: "reasoning",
    summary: reasoning,
    replay: replay(model, { reasoning }),
  };
}

function assistantItem(model: string, text: string): ProviderOutputItem {
  return {
    type: "assistant_message",
    text,
    replay: replay(model, { content: text }),
  };
}
...
async function* streamChat(
  ...
  const response = await streamFetch(body, profile, request.signal);
  if (!response.body)
  ...
  try {
    ...
    streamError(error, request.signal);
  }
  ...
  if (reasoning)
    yield {
      type: "item_done",
      item: reasoningItem(request.model, reasoning),
    };
  if (text)
    yield {
      type: "item_done",
      item: assistantItem(request.model, text),
    };
  for (const call of [...calls.entries()]
    ...
    yield {
      type: "item_done",
      item: toolCallItem(request.model, call.id, call.name, call.arguments),
    };
  }
...
function buildResponsesBody(request: StreamRequest): string {
...
function buildAnthropicBody(request: StreamRequest): string {
  return JSON.stringify({
    model: request.model,
      ...
      switch (event.type) {
        ...
        case "block_stop": {
          ...
          if (block.type === "tool_use") {
            ...
            yield {
              type: "item_done",
              item: assistantItem(request.model, block.text),
            };
          }
          break;
        }
...
export async function* streamResponse(
  ...
  switch (profile.protocol) {
    case "responses":
      yield* streamResponses(profile, request);
      return;
    case "anthropic-messages":
      yield* streamAnthropic(profile, request);
      return;
    case "chat-completions":
      yield* streamChat(profile, request);
      return;
  }
}
