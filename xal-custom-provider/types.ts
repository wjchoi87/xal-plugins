...
export interface ApiKeyCredential {
  ...
  key: string;
  /**
   * Per-profile endpoint config for the custom provider. Xal persists and
   * returns the credential object verbatim, so `xal connect custom <name>`
   * can create several profiles that each point at a different base URL and
   * wire protocol. When absent, the plugin falls back to `pluginConfig.custom`
   * for backward compatibility with install-time configuration.
   */
  baseUrl?: string;
  protocol?: string;
}
